...
Constant *CreateGlobalCounter(Module &M, StringRef GlobalVarName) {
  ...
  Constant *NewGlobalVar = M.getOrInsertGlobal(GlobalVarName, Type::getInt32Ty(CTX));
...
bool DynamicCallCounter::runOnModule(Module &M) {
  ...
  for (auto &F : M) {
    ...
    std::string CounterName = "CounterFor_" + std::string(FuncName);
    Constant *GV = CreateGlobalCounter(M, CounterName);
    CallCounterMap[FuncName] = GV;
    // Create a global variable to hold the name of this function
    auto FuncNameGV = Builder.CreateGlobalStringPtr(F.getName());
    FuncNameMap[FuncName] = FuncNameGV;

    // Inject instruction to increment the call count each time this function
    // executes
    LoadInst *load2 = Builder.CreateLoad(GV);
    Value *value = Builder.CreateAdd(Builder.getInt32(1), load2);
  ...
  // ------------------------------------------------------------------------
  Constant *ResultFormatStr = ConstantDataArray::getString(CTX, "%-20s %-10lu\n");
  Constant *ResultFormatStrVar = M.getOrInsertGlobal("ResultFormatStrIR", ResultFormatStr->getType());
  GlobalVariable *ResultGV = dyn_cast<GlobalVariable>(ResultFormatStrVar);
  ResultGV->setInitializer(ResultFormatStr);
  std::string out = "";
  out += "=================================================\n";
  ...
  Function *func = dyn_cast<Function>(M.getOrInsertFunction("printf_wrapper", FT).getCallee());
  
  // Create the entry basic block for printf_wrapper ...
  BasicBlock  *RetBlock = BasicBlock::Create(CTX, "enter", func);
